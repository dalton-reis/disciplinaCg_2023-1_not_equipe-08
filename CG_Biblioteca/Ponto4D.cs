...
namespace CG_Biblioteca
{
    ...
    public class Ponto4D
    {
        ...
        public bool IsDentroRangePonto(Ponto4D ponto)
        {
            double inicioX = this.X - 0.1;
            double fimX = this.X + 0.1;
            double inicioY = this.Y - 0.1;
            double fimY = this.Y + 0.1;

            bool dentroRangeX = (ponto.X > inicioX) && (ponto.X < fimX);
            bool dentroRangeY = (ponto.Y > inicioY) && (ponto.Y < fimY);

            return dentroRangeX && dentroRangeY;
        }
